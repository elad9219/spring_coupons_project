...
public interface CouponRepository extends JpaRepository<Coupon, Integer> {
    ...
    @Query(value = "SELECT COUNT(*) FROM customers_vs_coupons WHERE customer_customer_id = ? " +
                   "AND coupons_coupon_id = ?", nativeQuery = true)
    int checkCustomerSameCoupon(int customer_id, int coupon_id);


    @Transactional
    @Modifying
    @Query(value = "INSERT INTO customers_vs_coupons (customer_customer_id, coupons_coupon_id) " +
    ...
    @Query(value = "SELECT * FROM coupons INNER JOIN customers_vs_coupons " +
                   "ON coupons.coupon_id = customers_vs_coupons.coupons_coupon_id " +
                   "WHERE customers_vs_coupons.customer_customer_id = ?", nativeQuery = true)
    ...
    @Query(value =  "SELECT * FROM coupons INNER JOIN customers_vs_coupons " +
                    "ON coupons.coupon_id = customers_vs_coupons.coupons_coupon_id " +
                    "WHERE customers_vs_coupons.customer_customer_id = ?1 AND price <= ?2", nativeQuery = true)
